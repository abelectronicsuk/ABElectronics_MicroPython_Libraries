...
class IOPi(object):
    ...
    # interrupt occurred.
    INTCAPB = const(0x11)
    GPIOA = const(0x12)  # data port A
    GPIOB = const(0x13) # data port B
    OLATA = const(0x14)  # output latches A
    OLATB = const(0x15)  # output latches B
    ...
    def __init__(self, address, sda=None, scl=None, initialise=True):
        """
        ...
        self.__bus.writeto_mem(self.__ioaddress, self.IOCON,bytearray([self.__conf]))
        if initialise is True:
            self.__bus.writeto_mem(self.__ioaddress, self.IODIRA,bytearray([0xFF],[0xFF]))
            self.__bus.writeto_mem(self.__ioaddress, self.GPPUA,bytearray([0x00],[0x00]))
            self.__bus.writeto_mem(self.__ioaddress, self.IPOLA,bytearray([0x00],[0x00]))
    ...
    def __get_smbus(bus):
        ...
        sdaPIN=machine.Pin(20)
        sclPIN=machine.Pin(21)
        return machine.I2C(0,sda=sdaPIN, scl=sclPIN, freq=400000)
    ...
    def __get_bus(self, a_register):
        """
        Internal method for reading a 16-bit value from
        two consecutive device registers
        ...
        :raises ValueError: value out of range: 0 to 65535 (0xFFFF)
        """
        a = self.__bus.readfrom_mem(self.__ioaddress, a_register,2)
        b = int.from_bytes(a, 'big', False)
        return b
    ...
    def get_bus_direction(self):
        ...
        :rtype: int
        """
        return self.__get_bus(self.IODIRA)
    ...
    def get_bus_pullups(self):
        ...
        :rtype: int
        """
        return self.__get_bus(self.GPPUA)
    ...
    def read_bus(self):
        ...
        :rtype: int
        """
        return self.__get_bus(self.GPIOA)
    ...
    def get_bus_polarity(self):
        ...
        :rtype: int
        """
        return self.__get_bus(self.IPOLA)
    ...
    def mirror_interrupts(self, value):
        ...
        conf = self.__bus.readfrom_mem(self.__ioaddress, self.IOCON)

        if value == 0:
            conf = self.__updatebyte(conf, 6, 0)
            self.__bus.writeto_mem(self.__ioaddress, self.IOCON, bytearray([conf]))
        if value == 1:
    ...
    def get_interrupt_polarity(self):
        ...
        :rtype: int
        """
        return self.__checkbit(self.__bus.readfrom_mem(self.__ioaddress,
                                                         self.IOCON), 1)
    ...
    def get_interrupt_on_bus(self):
        ...
        :rtype: int
        """
        return self.__get_bus(self.GPINTENA)
